...
select parent_bss.name as parent_bss, casedw.brain, spec.specimen_id, tissue.name as tissue, specdw.ischemic_time as ischemic_time, 
       latestRin(substr(spec.Specimen_ID,1,14)||'26') as latestRin, autolysis as autolysis, circ.hardy_scale
  from cdrds.dr_case                 c,
 ...
 where pspec.specimen_record_id = spec.id       and
       ...
       bss.parent_bss_id = parent_bss.id        
       /* and
       c.id in 
       (select distinct persisted_object_id 
          from cdrds.audit_log 
