create or replace function LatestRin(Specimen_ID varchar2) return varchar2 is
  Result varchar2(15);
  runDate1 date;
  runDate2 date;
  ...
  latestRunDate date;
   ...
   cursor v_ldacc_specimen (Spec_rec_ID varchar2) is
     select lspec.id, lspec.specimen_record_id
      ...
      where lspec.specimen_record_id = Spec_rec_ID
      ;
...
begin
  ...
  problemFound := 'false';
  for spec_rec in v_dr_specimen(Specimen_ID)
    loop
      for acquis_rec in v_st_acquis_type(spec_rec.tissue_type_id)
        loop
          if (acquis_rec.code = 'BRAIN') then
            return null;
          end if;
        end loop;
      for lspec_rec in v_ldacc_specimen(spec_rec.id)
        loop
