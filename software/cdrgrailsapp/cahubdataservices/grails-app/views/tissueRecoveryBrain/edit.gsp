                  ...
                  <!--    <th class="nowrap">Interval<span data-msg="Time (hh:mm) between Brain removal and tissue sample dissection" class="cahub-tooltip"></span></th> -->
                    ...
                    <g:each in="${brainTissues}" status="i" var="t">
                      <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                        ...
                        <td class="textcenter"><g:textField id="time_${i}" name="time_${t.id}" value="${t.collectionTimeStr}" class="timeEntry ${errorMap.get('time_' + t.id)} recordinterval"  /><g:if test="${i==16 || i==17}"><span class="button"><g:actionSubmit id="clear_${i}"  class="date-time-clear"  value="Clear" /></span></g:if></td>
                        <td>${t.tissueType.name}</td>
                        <td><g:textArea id="comments_${t.id}" name="notes_${t.id}" value="" style="height:28px;width:150px;" value="${t.notes}" /></td>
