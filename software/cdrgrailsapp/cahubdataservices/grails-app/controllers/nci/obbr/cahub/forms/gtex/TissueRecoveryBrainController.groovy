...
class TissueRecoveryBrainController {
    ...
    Map checkError(tissueRecoveryBrainInstance){
        ...
        list.each{
            ...
            def notes = it.notes
            if(!mass && it.position != 'B08' && it.position != 'B09' ){
                result.put("mass_" + it.id, "The mass for position " + it.position + " is a required field")
            }
            if(!collectionDateStr && it.position != 'B08' && it.position != 'B09'){
            ...
            if(!collectionTimeStr && it.position != 'B08' && it.position != 'B09'){
                result.put("time_" + it.id, "The time for position " + it.position + " is a required field")
            }
            
            if(it.position == 'B08' || it.position == 'B09' ){
                // println("mass: " + mass + "  collectionDateStr: "  + collectionDateStr  + " collectionTimeSt: " + collectionTimeStr)
                if(!(mass && collectionDateStr && collectionTimeStr ) &&(mass || collectionDateStr || collectionTimeStr || notes) ){
                    //  println("I am here")
                    result.put(it.position, "Please make sure the data for position " + it.position + " is all or nothing.")
