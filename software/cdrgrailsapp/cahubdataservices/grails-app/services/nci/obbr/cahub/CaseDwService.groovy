...
class CaseDwService {
    ...
    def populateCaseDw(caseDwInstance) {
        ...
        def procDuration = 0
        def minFixTime=0, avgFixTime=0, maxFixTime=0, procedureDuration=0
        ...
        if (developmentFlag) { /* c.study.code='GTEX' and (c.phase.code='BP' or c.phase.code='IP' or c.phase.code='PP')" */
            ...
            gtexCases.each(){
                if (caseDwIncludeStatus.contains(it.caseStatus?.code)){
                    gtexCasesDev.add(it)
                }
            }
        }
...
//              println "(24 * 60 * 60 * 1000) = " + minFixTime
            ...
            skipCase = false
            xClampMillis = 0
            deathMap = gtexDonorVarsExportService.getDeathMap(it)  // the ....OPO means CHANGING HOW WE CALCULATE ISCHEMIC TIME WITH THIS RELEASE
