...
function aquaRefreshJs(){
    ...
    $("#aquaKit .td_tissueLocation select").focus(function () {
        ...
        if(parseInt($(this).val()) > 0) {
            updateTRFEntry( currentId, $(this).val(), 'tissueLocation.id'); 
        } else {
            updateTRFEntry( currentId, null, 'tissueLocation.id'); 
        }
    });
    ...
    $("#aquaKit .otherTissueLocation").change(function(){
        updateTRFEntry( $(this).data("currentid"), $(this).val(), 'otherTissueLocation');
    });
    ...
    $('#aquaKit .gtexQuickSave').blur(function() {
        updateTRFEntry( $(this).data("currentid"), $(this).val(), $(this).data("param"));
    });
    ...
    $('#aquaKit .td_tissueLocation select').blur(function() {
        if($(this).val() != 28 && $("#otherTissueLocation_" + $(this).data("currentid") ).val().length > 0) {
            updateTRFEntry( $(this).data("currentid"), "", 'otherTissueLocation');
...
function validateTime(obj) {
    var time = obj.value.substring(0,5);
    var convertedTime = (Number(time.split(':')[0])*60+Number(time.split(':')[1]))*1000;
}
...
function updateTRFEntry( currentId, currentVal, dataEntry) {
    jQuery.ajax({
        type: 'POST',
        data: dataEntry + '=' + currentVal + '&id=' + currentId, 
